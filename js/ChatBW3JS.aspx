...
function LastFunction() {setTimeout("resizeChatWin();", 500);
...
function showContent(i) {
	...
	if (i == 2 || i == 3) {
		if (wordscheckstring == null || wordscheckstring=='|') {hiddenContent();
			return;
		}
		if (uploading) {
...
function f20(html, type11, oname11, add) {
    if(type11>1)newmsg(html,v3,oname11);
	html = f20_1(html, type11, oname11);
	if (!isFocus) {
		window.focus();
	}
	if (add) {
...
function f20_1(content, pclass, ona) {
...
var clearhtml=false;
 function AddLine() {var doc = frames["FreeTextBox1_editor"].document, sel = doc.selection, rng;
            if (sel) {
                rng = sel.createRange();
                rng.pasteHTML('<br>');
                rng.move("character", 1);
