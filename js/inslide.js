(function($){
    $.fn.ckSlide = function(opts){
        ...
        this.each(function(){
            var slidewrap = $(this).find('.hz_in_tplh-wrapper');//轮播元素父对象
			...
			//点击切换相应序号的图片
            $(this).find('.hz_in_tplhbox li').each(function(cindex){
            ...
            $(this).on('mouseleave', function(){
                ...
                $(this).find('.ctrl-slide').css({opacity:0.8});
            });
            startAtuoPlay(opts.interval);
            // 自动滚动播放
            ...
            // 修正box  标记居中
            var box = $(this).find('.hz_in_tplhbox');
            box.css({
            ...
            switch(opts.dir){
                case "x":
					...
					//.wrap()包裹页面已经定义的.hz_in_tplh-wrapper以及子元素
    ...
    function change(show, hide){
        ...
        if(opts.dir == 'x'){
            ...
            $(this).find('.hz_in_tplh-wrapper').stop().animate({'margin-left':-x}, function(){opts['isAnimate'] = false;});
            opts['isAnimate'] = true;//图片在移动过程中设置按钮点击不可用，确保每一次轮播视觉上执行完成，
        }else if(opts.dir == 'y'){//垂直移动——自己添加
            var y = show * opts['height'];
            $(this).find('.hz_in_tplh-wrapper').stop().animate({'margin-top':-y}, function(){opts['isAnimate'] = false;});
            opts['isAnimate'] = true;
        }
		else{
			//默认的淡隐淡出效果
            $(this).find('.hz_in_tplh-wrapper li').eq(hide).stop().animate({opacity:0});
            $(this).find('.hz_in_tplh-wrapper li').eq(show).show().css({opacity:0}).stop().animate({opacity:1});
        }
       //切换对应标记的颜色
        $(this).find('.hz_in_tplhbox li').removeClass('current');
        $(this).find('.hz_in_tplhbox li').eq(show).addClass('current');
    }
    $.fn.ckSlide.opts = {
